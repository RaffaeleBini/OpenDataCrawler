...
**"Piloto: Servicio Sanitarios de Galicia (mapa)"**, con un **mapa de puntos** sobre el dataset "Servicio Sanitarios de Galicia" (opendata.esri.es): 2.031 puntos (centros sanitarios, puntos de donación de sangre, entre otros) con coordenadas `X`/`Y`.

- **No son coordenadas geográficas (longitud/latitud), sino proyectadas** — un caso más avanzado que el de Alemania, donde no bastaba con partir un texto: hacía falta una reproyección cartográfica real. Se comprobó primero, sin éxito, asumiendo UTM huso 29N (el mismo sistema que usa el IGN para los límites de concellos/comarcas ya integrados) — los puntos resultantes caían fuera de Galicia. Consultando los metadatos reales del servicio ArcGIS de origen (`.../FeatureServer/1?f=json`, campo `extent.spatialReference.wkid`) se confirmó que el sistema real es **EPSG:25830 (ETRS89 / UTM huso 30N)**, no el 29N — un detalle que no aparece en ningún sitio visible del propio dataset dentro del catálogo, así que sin comprobar los metadatos del origen se habría dado por buena una reproyección incorrecta.
- La base de datos del DataStore no tiene la extensión PostGIS instalada, así que no hay una función `ST_Transform` disponible dentro de una consulta SQL de Metabase para hacer la reproyección en el momento (a diferencia del caso alemán, que solo necesitaba separar un texto). En su lugar, se calcularon las coordenadas en longitud/latitud reales para las 2.031 filas fuera de Metabase, con `ogr2ogr` (mismo programa ya usado para los mapas de comarcas/concellos), y se incorporaron a la pregunta como una tabla de valores (`VALUES`) cruzada por `OID` dentro de la propia consulta SQL nativa — el dato de origen (`X`/`Y` tal cual los da la fuente) no se toca en ningún momento.
- Límite práctico a tener en cuenta: las preguntas nativas de Metabase devuelven como máximo 2.000 filas por defecto; el dataset tiene 2.031, así que el mapa muestra los primeros 2.000 puntos, no los 2.031. No bloqueante para un mapa exploratorio.
...
## Mapas de región de Galicia (comarca y concello)

Metabase trae de serie mapas de región de países y de EE. UU., pero no de las provincias, comarcas o concellos gallegos. Hay dos mapas personalizados registrados en Metabase para cubrir ese hueco:

### Por comarca

El usuario aportó un GeoJSON propio (límites de las 53 comarcas de Galicia, en WGS84), registrado en Metabase como "Comarcas de Galicia".
...
- Piloto de verificación: dashboard "Piloto: Comarcas de Galicia (mapa de rexión)", con un mapa de las 53 comarcas coloreadas por su superficie en km² (dato tomado del propio GeoJSON, sin depender de ningún dataset del catálogo) — confirma que el mapa se registra y las 53 comarcas casan por nombre. Ningún dataset del catálogo tiene hoy una columna de comarca (ver mapa por concello, más abajo, para un piloto con datos reales).

### Por concello

Límites de los 313 concellos de Galicia, descargados del [centro de descargas de la Xunta de Galicia](https://mapas.xunta.gal/es/mapas/informacion-geografica/centro-de-descargas) (fuente: Instituto Geográfico Nacional de España), reproyectados de ETRS89/UTM 29N a WGS84 con `ogr2ogr`. Registrado en Metabase como "Concellos de Galicia".

- El fichero se sirve desde `/concellos-galicia.geojson` y vive también en GitHub.
- **Licencia**: cesión exclusivamente para usos no comerciales, con la obligación de citar como propietario al Instituto Geográfico Nacional de España (fórmula: "copyright, Instituto Geográfico Nacional de España"). Encaja con el uso de DATAlife (EDIH, sin ánimo comercial), pero cualquier difusión de este mapa tiene que respetar esa cita.
- La columna de cruce (`CONCELLO_MAIUS`) está normalizada a mayúsculas y con el artículo al principio (p. ej. "O GROVE", no la forma administrativa "Grove, O" que trae el fichero original de la Xunta) para casar con el campo `concello` de los datasets ya conectados (MeteoGalicia, por ejemplo).
- **Piloto de verificación con datos reales del catálogo**: dashboard "Piloto: choiva media por concello (MeteoGalicia)", con la choiva media diaria por concello calculada sobre el histórico ya acumulado (filtrando `lnCodigoValidacion IN (1, 5)`, es decir, solo datos válidos, para no mezclar el valor centinela `-9999` de "dato non rexistrado" en la media — se detectó exactamente ese problema al construir el piloto, con concellos dando medias negativas de -300&nbsp;L/m², y se corrigió antes de darlo por bueno).
- **Cobertura real del cruce por nombre, no inflada**: de los 136 concellos con estación de MeteoGalicia, 133 (98&nbsp;%) casan por nombre con el GeoJSON tras la normalización. Quedan 3 sin casar por variantes reales de nombre entre las dos fuentes, no arregladas automáticamente: "Cangas" (MeteoGalicia) frente a "Cangas de Morrazo" (IGN), "Castro Caldelas" frente a "Castro de Caldelas", y "Riós" frente a "Riós, O". Arreglarlo del todo requeriría una tabla de alias manual; no se ha hecho por ahora.

**Gotcha importante de seguridad, a tener en cuenta para cualquier mapa personalizado futuro**: Metabase bloquea por diseño cualquier URL de GeoJSON personalizado que resuelva a una dirección de red privada o interna (protección contra SSRF, ver [GHSA-w73v-6p7p-fpfr](https://github.com/metabase/metabase/security/advisories/GHSA-w73v-6p7p-fpfr)) — probado y confirmado que rechaza tanto el nombre de host interno de Docker (`ckan`) como la IP privada del contenedor, sin ninguna variable de entorno para permitirlo (solo existe `MB_CUSTOM_GEOJSON_ENABLED` para desactivar la función entera, no para relajar esta restricción). Por eso ninguno de los dos GeoJSON se sirve desde la red interna de Docker, sino desde una URL realmente pública en GitHub (`raw.githubusercontent.com/RaffaeleBini/OpenDataCrawler/main/...`).
