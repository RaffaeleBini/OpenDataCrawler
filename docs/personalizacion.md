...
- **Portada**: la caja de relleno por defecto de CKAN ("This is a featured section", plantilla `home/snippets/promoted.html`) se sustituyó por el logotipo completo, enlazando a `dihdatalife.com` (override de esa plantilla en `ckanext-datalifetheme`, requiere reconstruir la imagen `ckan`). El párrafo de bienvenida se configura con `ckan.site_intro_text` (editable en caliente, igual que `ckan.site_custom_css`).

**Advertencia práctica sobre `curl` en este entorno (Windows/Git Bash)**: al fijar `ckan.site_custom_css` o `ckan.site_intro_text` con `curl --data-binary @fichero.json` sobre un fichero con acentos/eñes, el texto llegó corrompido al guardarse, aunque el fichero en disco tenía los bytes UTF-8 correctos. Usar en su lugar una petición hecha con Python (`urllib.request`), que sí conserva la codificación correctamente.
